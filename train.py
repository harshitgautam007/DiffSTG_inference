...
def default_config(data='AIR_BJ'):
    ...
    if config.data.name == 'PM25':
        # Load your data file to get its dimensions automatically
        flow_data = np.load(config.data.path + 'PM25/flow.npy')
        total_timesteps, num_nodes = flow_data.shape

        config.data.num_features = 1
        config.data.num_vertices = num_nodes
        config.data.points_per_hour = 1 # 👈 Your data is hourly, so this is 1.

        # Define the train/validation/test split (e.g., 60% / 20% / 20%)
        config.data.val_start_idx = int(total_timesteps * 0.6)
        config.data.test_start_idx = int(total_timesteps * 0.8)
    if config.data.name == 'PM':
        # Load your data file to get its dimensions automatically
        flow_data = np.load(config.data.path + 'PM/flow.npy')
        total_timesteps, num_nodes = flow_data.shape
